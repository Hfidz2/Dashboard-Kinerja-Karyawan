...
with tab1:
    ...
    with col2:
        st.metric("Rata-rata Skor", round(df['Performance_Score'].mean(), 2))
    with col3:
        st.metric("Performa Maksimum", df['Performance_Score'].max())
    st.divider()
    ...
    with col_a:
        fig1 = px.histogram(
            df,
            x="Performance_Score",
            title="Distribusi Skor Performa",
            color_discrete_sequence=["#1f77b4"] if not dark_mode else ["#00ff9f"]
        )
        fig1.update_layout(
    ...
    with col_b:
        fig2 = px.pie(df, names="Performance_Score", title="Proporsi Skor Performa", color_discrete_sequence=px.colors.sequential.Blues)
