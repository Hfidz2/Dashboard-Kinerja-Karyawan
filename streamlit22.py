...
def tab_header(title, subtitle):
    st.markdown(f"""
    <div style='border: 2px solid #1f77b4
...
with tab1:
    ...
    with col_a:
        st.markdown(f"""
        <div style='border: 2px solid #1f77b4; padding: 15px; border-radius: 15px; 
                margin-bottom: 20px; background-color: {background_color_anakan}; color: {font_color};'>
        ...
        fig1 = px.histogram(
            ...
            x="Performance_Score",
            title=" ",
            color_discrete_sequence=["#1f77b4"] if not dark_mode else ["#1f77b4"]
        )
        fig1.update_layout(
            ...
            xaxis=dict(
                title="Skor Performa",
                color=font_color,
                gridcolor="#444" if dark_mode else "#ccc"
            ),
            yaxis=dict(
    ...
    with col_b:
        ...
        fig2 = px.pie(df, names="Performance_Score", title=" ", color_discrete_sequence=px.colors.sequential.Blues)
        fig2.update_layout(paper_bgcolor=background_color_anakan, font=dict(color=font_color))
    ...
    with col_f:
        ...
        if mode == "Rata-rata Bulanan":
            ...
            fig4 = px.line(line_data, x='Hire_Month', y='Performance_Score', title="Rata-rata Skor Per Bulan")
            fig4.update_layout(paper_bgcolor=background_color_anakan, plot_bgcolor=background_color_anakan, font=dict(color=font_color))
            st.plotly_chart(fig4)
        else:
            top_dept = df['Department'].value_counts().head(3).index
            fig5 = go.Figure()
            for dept in top_dept:
                data = df[df['Department'] == dept].groupby('Hire_Month')['Performance_Score'].mean().reset_index()
                fig5.add_trace(go.Scatter(x=data['Hire_Month'], y=data['Performance_Score'], mode='lines+markers', name=dept))
            fig5.update_layout(title="Top 3 Divisi - Tren Skor Performa", xaxis_title="Bulan", yaxis_title="Skor",
                               paper_bgcolor=background_color_anakan, plot_bgcolor=background_color_anakan, font=dict(color=font_color))
    ...
    with col_g:
        ...
        col_e = st.selectbox("Pilih Fitur Numerik:", ['Monthly_Salary', 'Work_Hours_Per_Week', 'Projects_Handled',
                                                      'Overtime_Hours', 'Sick_Days', 'Training_Hours',
                                                      'Employee_Satisfaction_Score'])
        col_e_avg = df.groupby("Performance_Score")[col_e].mean().reset_index()
        fig3 = px.bar(col_e_avg, x="Performance_Score", y=col_e, title=f"Rata-rata {col_e} per Skor Performa",
                     color_discrete_sequence=["#1f77b4"])
        fig3.update_layout(paper_bgcolor=background_color_anakan, plot_bgcolor=background_color_anakan, font=dict(color=font_color))
...
with tab2:
    ...
    with col3:
        tab_header("Distribusi Fitur Berdasarkan Performance Score", "Distribusi Gaji per Skor Performa")

        fig_a = px.box(filtered_df, x="Performance_Score", y="Monthly_Salary", title=" ")
    ...
    with col4:
        tab_header("Distribusi Fitur Berdasarkan Performance Score", "Distribusi Kepuasan per Skor Performa")
        fig_b = px.box(filtered_df, x="Performance_Score", y="Employee_Satisfaction_Score", title="")
...
with tab5:
    ...
    with st.container():
        tab_header("Fitur-Fitur Penting dalam Prediksi", "Pentingnya Setiap Fitur dalam Model (Skala 1-10)")
    ...
    with st.container():
        tab_header("Fitur-Fitur Penting dalam Prediksi", "WordCloud Feature Importance")
    ...
    with st.container():
        tab_header("Fitur-Fitur Penting dalam Prediksi", "Donut Chart Feature Importance")
        fig_donut = px.pie(
            imp_df,
            names="Fitur",
            values="Pentingnya",
    ...
    with st.container():
        tab_header("Fitur-Fitur Penting dalam Prediksi", "Top 5 Fitur Paling Penting")
